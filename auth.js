/* ================================
   ...
   (ES module — imports everything from firebase.js)
   ================================ */
import {
...
function userRef(uid) { return ref(db, 'users/' + uid); }
async function getUserData(uid) { const snap = await get(userRef(uid)); return snap.exists() ? snap.val() : null; }
...
async function saveNewUsername() {
  ...
  try {
    ...
    await update(userRef(user.uid), { username: val, updatedAt: Date.now() });
